function [ scribbles ] = freehand_scribble( scan, mask_gt, mask_scribble, current_slice, varargin)
%% AUTHOR: Basil Mustafa (BasilAMustafa@gmail.com)
% 
...
while sliceNo <= slice_count
    ...
    title(['Annotating slice ' num2str(current_slice + sliceNo - 1)]);
    ...
    while lab_idx <= numel(labels)
        ...
        if ~go_to_previous_slice 
            ...
            if current_label == bg_label
                ...
                %if not background scribble, we only 
            ...
            elseif k_press == 'w' || k_press == 's'
                ...
                set(mTextBox,'string', {sprintf('SCRIBBLING ON LABEL %s',num2str(current_label)); ...
                                        'Press ESC to cancel'});
                set(mTextBox,'Position',[t_dims(1), t_dims(2), 25, 2]);
                end
                lab_idx = lab_idx - 1;
